...
7. **Review-fraud defenses** (build the storage in M3, the scoring in M4). Reuse-first: **card fingerprints and charge risk scores come from Stripe/Radar** (read `payment_method.card.fingerprint`; do not build payment fingerprinting); **device fingerprinting via ThumbmarkJS** (MIT — NOT FingerprintJS, whose current OSS version is BSL and not free for production): store the hash per session, correlate with SQL; **review-text similarity via pgvector** (already in Supabase) over embeddings; tripwire rules at MVP are ~5 SQL queries on a nightly cron — adopt **Marble** (open-source self-hosted rules engine + case manager) only when rules multiply post-launch; **skip academic fake-review ML models** — behavioral defect-rate ranking replaces what text classifiers exist to catch. Details:
   ...
   - **Ranking score ≠ displayed average — and negatives outweigh praise.** Positive ratings inflate toward uselessness (the 4.9-everywhere problem); the discriminating signal is the negative tail, which our paid-review gate defends structurally (sabotaging a rival costs full price per attempt, through a fingerprinted card, adjudicated by admin). Rank primarily on **defect rate per completed engagement** — refunds/disputes, non-repeat rate, thread abandonment, review-declined rate, reports/blocks — then repeat rate, then clean volume, then credibility-weighted stars (weights: reviewer account age, payment-method uniqueness, multi-coach breadth, engagement depth, outcome-tag presence). Bayesian-shrink defect rates for low-volume coaches so 10 clean transactions don't outrank 2 defects in 1,400. Volume-with-clean-record is the currency a coach cannot purchase: manufacturing it means processing thousands of real-fee transactions.
   - **Surface the critical tail**: coach profiles show a "most critical recent" review alongside the latest — unburied negatives build buyer trust and let good coaches respond publicly; hiding them is what makes walls of five-stars read as fake.
